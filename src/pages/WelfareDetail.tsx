...
import { ChevronLeft, Bookmark, Share2, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
...
export function WelfareDetail({ onNavigate }: WelfareDetailProps) {
  ...
  const faqs = [
    {
      ...
      answer: '만 65세 이상 노인 또는 65세 미만이지만 노인성 질병을 가진 분들이 신청 가능합니다.',
    },
    {
      question: '어떻게 신청하나요?',
      answer: '국민건강보험공단 지사 방문 또는 우편, 팩스, 인터넷(www.longtermcare.or.kr)을 통해 신청 가능합니다.',
    },
    {
      question: '필요한 서류는 무엇인가요?',
      answer: '장기요양인정신청서, 의사소견서(등급판정 후 제출 가능)가 필요합니다.',
    },
    {
      question: '비용은 얼마나 드나요?',
      answer: '본인부담금은 서비스 비용의 15~20%���며, 의료급여 수급자는 면제 또는 경감됩니다.',
    },
  ];
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <Card variant="elevated" padding="lg" className="mb-6">
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              <Badge variant="level2" size="sm">노인돌봄</Badge>
              <Badge variant="default" size="sm">65세 이상</Badge>
            </div>
            ...
            >
              <p className="text-sm" style={{ color: colors.textDark }}>
                <strong>AI 요약:</strong> 고령이나 노인성 질병 등으로 일상생활을 혼자서 수행하기 어려운 노인 등에게 
                신체활동 또는 가사활동 지원 등의 장기요양급여를 제공하여 노후의 건강증진 및 
                생활안정을 도모하고 그 가족의 부담을 덜어주는 제도입니다.
              </p>
            </div>
          </div>
        </Card>
          ...
          <ul className="space-y-2">
            <li className="flex items-start space-x-2">
              ...
              <span className="text-sm" style={{ color: colors.textSub }}>
                만 65세 이상 노인 또는 65세 미만이지만 치매, 뇌혈관질환 등 노인성 질병을 가진 자
              </span>
            </li>
            <li className="flex items-start space-x-2">
              <span style={{ color: colors.mainGreen2 }}>•</span>
        ...
        <Card variant="elevated" padding="lg" className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {['방문요양', '방문목욕', '방문간호', '주·야간보호', '단기보호', '복지용구'].map((service) => (
              <div 
                key={service}
